...
class WhisperSTT {
  ...
  Future<void> load() async {
    try {
      ...
      final encoderFile = File(mm.whisperEncoderPath);
      final decoderFile = File(mm.whisperDecoderPath);
  ...
  // lib/services/whisper_stt.dart
  // Update the _transcribe method to handle errors better

  String _transcribe(Float32List audio, String language) {
    if (_encoderSession == null || _decoderSession == null) {
      throw Exception('Whisper sessions not initialized');
    }
    ...
    try {
      ...
      for (int step = 0; step < 224; step++) {
        ...
        final idData =
            Int64List.fromList(feedIds.map((e) => e.toInt()).toList());

        final idTensor =
            OrtValueTensor.createTensorWithDataList(idData, [1, decLen]);
      ...
      encHidden.release();
      final result = _decodeTokens(decoded);
      print('Transcription result: "$result"');
      return result;
    } catch (e) {
      print('Error in _transcribe: $e');
      return '';
    }
  }
  // ── Mel spectrogram ───────────────────────────────────────────────────────
  ...
  Float32List _computeMelSpectrogram(Float32List audio) {
    ...
    for (int i = 0; i < reflPad; i++)
      sig[targetLen + reflPad + i] = padded[targetLen - 2 - i];
    ...
    while (len <= n) {
      ...
      for (int i = 0; i < n; i += len) {
        ...
        for (int k = 0; k < half; k++) {
          ...
          final vRe = buf[2 * (i + k + half)],
              vIm = buf[2 * (i + k + half) + 1];
          final tvRe = cRe * vRe - cIm * vIm, tvIm = cRe * vIm + cIm * vRe;
          buf[2 * (i + k)] = uRe + tvRe;
          buf[2 * (i + k) + 1] = uIm + tvIm;
          buf[2 * (i + k + half)] = uRe - tvRe;
          buf[2 * (i + k + half) + 1] = uIm - tvIm;
          final nc = cRe * wRe - cIm * wIm;
          cIm = cRe * wIm + cIm * wRe;
          cRe = nc;
        }
      }
  ...
  Float32List _buildMelFilterbank() {
    ...
    final melPts = List.generate(
        _melBins + 2, (i) => melMin + i * (melMax - melMin) / (_melBins + 1));
    final bins = List.generate(_melBins + 2, (i) {
      return (_melToHz(melPts[i]) / fMax * _nFreqBins)
          .round()
          .clamp(0, _nFreqBins - 1);
    });
  ...
  Map<String, int> _buildUnicodeToByte() {
    ...
    return {
      for (int i = 0; i < direct.length; i)
        String.fromCharCode(cs[i]): direct[i]
    };
  }
